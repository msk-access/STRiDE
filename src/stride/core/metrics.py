...
def compute_metrics(
    ...
    out = {
        "accuracy": (
            float(accuracy_score(y_true, y_pred)) if accuracy_score else derived["accuracy"]
        ),
        "precision": (
            float(precision_score(y_true, y_pred, zero_division=0))
            if precision_score
            else derived["precision"]
        ),
        "recall_sensitivity": (
            float(recall_score(y_true, y_pred, zero_division=0))
            if recall_score
            else derived["sensitivity"]
        ),
        "specificity": (
            float(recall_score(y_true, y_pred, pos_label=0, zero_division=0))
            if recall_score
            else derived["specificity"]
        ),
        "balanced_accuracy": (
            float(balanced_accuracy_score(y_true, y_pred))
            if balanced_accuracy_score
            else derived["balanced_accuracy"]
        ),
        "f1_score": (
            float(f1_score(y_true, y_pred, zero_division=0)) if f1_score else derived["f1_score"]
        ),
    }
